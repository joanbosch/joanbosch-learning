...
<Callout emoji="">
Les estructures condicionals ens permeten executar una instrucció o una altra depenent d'una determinada condició. En Python la instrucció condicionals s'escriu usant les paraules clau `if` i `else` utilitzant el següent format:
</Callout>
...
En alguns casos, ens podem trobar amb la casuística en què haurem d'encadenar algunes condicions. Això ho podem aconseguir afegint una nova estructura condicional dins d'un dels dos blocs d'instruccions ⟨Instruccions_SÍ⟩ i ⟨Instruccions_NO⟩ així com es pot observar al següent exemple.
...
Ara bé, sovint ens pot interessar fer servir l'expressió `elif` que ens serveix per encadenar condicions. Llavors, podríem encadenar tantes condicions `elif` com vulguéssim.
```python
if ⟨Condició_1⟩:
    ⟨Instruccions_SÍ_1⟩
elif ⟨Condició_2⟩:
...
Per tant, el codi que hem vist abans, el podríem simplificar de la següent manera fent servir l'expressió `elif`:
```python
a = int(input())
